...
def bf_negative_cycle_test(graph):
    """
    Description
    -------
    Get the shortest path using the Bellman-Ford algorithm, and len short path for test
    ...
    v = x
    while True:
        len_short_path = len_short_path + 1
        cycle.append(int(v))
        if v == x and len(cycle) > 1:
            break
        v = p[int(v)]
    return list(reversed(cycle)), len_short_path
